...
- Documentation: Generate SVGs from `.mmd` files and build DOCX documentation (matching Eudorus).
- Testing: `uv run pytest` with `CI=1 UI_HEADLESS=1`; optional coverage artifact; temp dirs per job to isolate data.
- Quality: `uv run ruff check .`, `uv run ruff format --check .` for formatting drift, and `uv run pyright` (or equivalent) for strict type checking.

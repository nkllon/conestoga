...
from .events import EventDraft, FallbackDeck, Choice, Effect, EffectType, Outcome, Prerequisite, EventResolution
...
class GeminiGateway:
    ...
    def generate_event_resolution(
        ...
        try:
            ...
            resolution = self._parse_event_resolution(response.text, choice_id)
            if resolution:
...
- Resources: Food={game_state.food}, Money=${game_state.money}, Water={game_state.water}, Ammo={game_state.ammo}
...
3. Uses ONLY these effect types: adjust_resource, add_item, remove_item, adjust_health, adjust_morale
...
IMPORTANT: Return ONLY the JSON object, no markdown formatting, no explanation."""
    ...
    def _build_resolution_prompt(self, event: EventDraft, choice_id: str, game_state: GameState) -> str:
...
CURRENT STATE:
- Food: {game_state.food}, Money: ${game_state.money}, Water: {game_state.water}, Ammo: {game_state.ammo}
- Inventory: {', '.join(game_state.inventory.keys()) if game_state.inventory else 'empty'}

Generate a resolution with:
1. Narrative outcome (2-3 sentences)
2. Concrete effects using ONLY these operations: add_item, remove_item, modify_resource, modify_stat, set_flag
...
IMPORTANT: Return ONLY the JSON object."""
    ...
    def _parse_event_draft(self, response_text: str, item_catalog: ItemCatalog) -> Optional[EventDraft]:
    ...
    def _parse_event_resolution(self, response_text: str, choice_id: str) -> Optional[EventResolution]:
        ...
        try:
            ...
            for e_data in data['effects']:
                try:
                    ...
                    # Skip invalid effects but continue
                    continue
    ...
    def _check_resource_exhausted(self, error: Exception) -> bool:
        ...
        if "RESOURCE_EXHAUSTED" in error_str or "429" in error_str or "quota" in error_str.lower():
            print("[Gemini] RESOURCE_EXHAUSTED detected - API quota exceeded. Disabling Gemini API.")

...
class GameUI:
    ...
    def draw_panel(
        self, x: int, y: int, width: int, height: int, color: tuple, border_color: tuple
    ):
    ...
    def render_travel_screen(self, game_state: GameState):
        ...
        self.draw_text(
            "The Oregon Trail Journey", self.small_font, WHITE, self.width // 2, 70, center=True
        )
        ...
        self.draw_text(
            ...
            f"Miles: {game_state.miles_traveled} / {game_state.target_miles}",
            self.heading_font,
            WHITE,
            70,
            y_offset,
        ...
        self.draw_text(
            f"Location: {game_state.biome.value.title()}  |  "
            f"Weather: {game_state.weather.value.title()}",
            self.body_font,
            LIGHT_GRAY,
            70,
            y_offset,
        ...
        self.draw_text(
            f"Food: {game_state.food}  Water: {game_state.water}  "
            f"Ammo: {game_state.ammo}  Money: ${game_state.money}",
            self.body_font,
            WHITE,
            70,
            y_offset,
        ...
        self.draw_text(
            f"Wagon Health: {game_state.wagon_health}%",
            self.body_font,
            WHITE if game_state.wagon_health > 50 else RED,
            70,
            y_offset,
        )
    ...
    def render_event_screen(
        ...
        for i, choice in enumerate(event.choices):
            ...
            self.draw_panel(
                ...
                choice_height,
                bg_color,
                YELLOW if is_selected else WHITE,
            )
    ...
    def render_resolution_screen(self, resolution_text: str):
        ...
        self.draw_text(
            "Press SPACE to continue...", self.body_font, YELLOW, self.width // 2, 600, center=True
        )
    ...
    def render_game_over_screen(self, game_state: GameState):
        ...
        y_offset += 40
        self.draw_text(
            f"Miles Covered: {game_state.miles_traveled}",
            self.body_font,
            WHITE,
            self.width // 2,
            y_offset,
    ...
    def render_inventory_screen(self, game_state: GameState, item_catalog: ItemCatalog):
        ...
        self.draw_text(
            "Press ESC to return", self.small_font, LIGHT_GRAY, self.width // 2, 680, center=True
        )

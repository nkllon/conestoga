...
from conestoga.game.events import EventResolution, Outcome, Effect, EffectType
from conestoga.game.state import GameState, PartyMember
...
class TestSkillCheck(unittest.TestCase):
    def test_random_skill_check_logic(self):
        ...
        self.assertEqual(result, "Failure", "Random check should fail against high DC regardless of fake skills")
    ...
    def test_normal_skill_check_logic(self):
        ...
        outcome = Outcome(
            text="Outcome",
            success_required={"skill": "hunter", "dc": 10}, # 10 + roll(1-20) >= 10 -> Always success
            success_text="Success",
            failure_text="Failure"
        )

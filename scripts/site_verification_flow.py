...
def get_authorized_session():
    credentials, project = google.auth.default(scopes=['https://www.googleapis.com/auth/siteverification'])
...
def verify_ownership(session, token):
    print(f"✨ Verifying ownership now...")
    resp = session.post(
...
def main():
    ...
    if verify_ownership(session, txt_record):
        ...
        print("⚠️ Verification API returned error, but we might proceed to mapping anyway as it might have worked/be async.")
